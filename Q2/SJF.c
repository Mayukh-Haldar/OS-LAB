...
int main() {
    ...
    // Reading process arrival times and burst times
    for (int i = 0; i < n; i++) {
    ...
    while (completed != n) {
        ...
        for (int i = 0; i < n; i++) {
            if (ps[i].at <= current_time && is_completed[i] == false) {
                ...
                if (ps[i].bt == minimum && ps[i].at < ps[min_index].at) {
                    minimum = ps[i].bt;
                    min_index = i;
                }
            }
        ...
        if (min_index == -1) {
            current_time++;  // Increment time if no process is ready to run
        } else {
            // Process has been selected, calculate its timings
    ...
    // Calculate the cycle length (from the earliest arrival to the latest completion)
    for (int i = 0; i < n; i++) {
