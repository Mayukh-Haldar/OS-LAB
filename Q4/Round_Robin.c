...
int comparatorAT(const void *a, const void *b) {
    int at_a = ((struct process_struct *)a)->at;
    int at_b = ((struct process_struct *)b)->at;
    if (at_a == at_b) {
...
int main() {
    ...
    // Sort structure based on Arrival time in increasing order (stable sort)
    ...
    while (completed != n) {
        ...
        // Check which new Processes need to be pushed to Ready Queue from Input list
        for (int i = 1; i < n; i++) {
    ...
    // Sort by PID to print output in the original order
    ...
    printf("\nCPU Utilization(Percentage)= %f", (float)(cpu_utilization * 100));
    return 0;
}
